        ...
        public function wpcf7_post_checkbox_shortcode_handler( $tag )
        {
            ...
            foreach ( $values as $key => $value ) {
                ...
                if ( $label_first ) { // put label first, input last
                    $item = sprintf(
                        '<span class="wpcf7-list-item-label">%1$s <i>%3$s</i></span><input %2$s />',
                        esc_html( $label ), $item_atts, $prices[$key] . ' грн' );
                } else {
                    $item = sprintf(
                        '<input %2$s /><span class="wpcf7-list-item-label">%1$s <i>%3$s</i></span>',
                        esc_html( $label ), $item_atts, $prices[$key] . ' грн' );
                }
